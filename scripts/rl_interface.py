...
class RLInterface(object):
  def __init__(self, prefix='', num_epsoide=1, time_per_ep=200, FPS=1000.0, save=False, verbose=False):
    ...
    # subscriber
    self.human_action_sub = rospy.Subscriber(prefix+'/delta_input_target_', Float64MultiArray, self.human_action_callback)
    self.joy_sub = rospy.Subscriber(prefix+'/joystick_command', Float64MultiArray, self.joy_command_callback)
  ...
  def loop(self, ob):
    ...
    ## data collection
    if self.save:
      transition = [ob, action, reward, ob_next, done]
      #transition = [reward]
...
def main():
  prefix = 'unity'
  FPS = 20
  rli = RLInterface(prefix=prefix, num_epsoide=1000, FPS=FPS, save=False)
  rate = rospy.Rate(FPS)
